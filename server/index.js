...
const con=mysql.createPool({
  ...
  app.get('/users/delete',(req,res)=>{
    con.query("TRUNCATE information",(err,result)=>{
...
app.post('/',async (req,response,next)=>{
  ...
  var value=await bcrypt.hash(pass,salt)
  
  const {email}=req.body;
  const data={ value,email }
 
con.query("insert into information (username,password) values (?,?)",[data.email,data.value],(err,result)=>{
      if(err){
